...
function renderMenu(categoryName) {
    ...
    categoryObj.items.forEach(item => {
        ...
        card.innerHTML = `
            <img src="assets/${item.id}.jpg" alt="${item.name}" class="item-img" onerror="this.src='https://placehold.co/200x150/1a5f5f/ff8c42?text=${encodeURIComponent(item.name)}'">
            <h3>${item.name}</h3>
...
function sendWhatsAppOrder() {
    ...
    let message = `🛒 *New Order – 0125 CAFE MYSTIC FALLS*%0A%0A`;
    let total = 0;
...
function sendBookingRequest() {
    ...
    bookingMessage += `🎈 *Event:* [Birthday / Anniversary / Kitty Party / Friends Meet]%0A`;
    bookingMessage += `👥 *Group Size:* [e.g. 10 People]%0A`;
